 ...
 * 
 * {@code available()} can be used to query for workflows that can be executed.
 * The returned {@see Workflow} instance can then be modified and executed with
 * {@code submit(Workflow)}. This returns an id that can be used to query the
...
public interface Submitter {
	 ...
	 *             if required parameters of the workflow are not set.
	 * @throws SubmitFailedException
	 * @throws RuntimeException
	 * @param workflow
	 *            The workflow that should be executed.
	 * @param foreignID
	 *            An optional id for this workflow run from another database.
	 *            This can be null.
	 * @param user
	 *            Execute the workflow as the specified user. // TODO String??
	 ...
	 * 
	 * @param id
	 *            The id of the workflow (not the foreign id, but the return
	 *            value of submit)
	 * @throws NoSuchElementException
	 *             If the id could not be found.
	 * @throws ConnectException
	 *             If the submitter could not connect to the workflow engine.
	 */
	 ...
	 *             If the workflow run is currently not running e.g. if it is
	 ...
	 * 
	 * @param Return
	 *            only a part (a page) of the available workflows. If you want
	 *            all workflows, set to -1
	 * @param sortKey
	 *            Return workflows sorted by sortKey.
	 * @return
	 * @throws ConnectException
	 * @throws IllegalArgumentException
	 *             if {@code page} is larger than the number of available pages
